# utils.py
# This is a collection of utility functions that are used by the BaseAgent.
# This is modified from the biomni package
# https://github.com/openbmb/Biomni
# https://github.com/openbmb/Biomni/blob/main/biomni/utils.py
...
import traceback
import zipfile
from typing import Any, ClassVar, Optional, get_type_hints, Union, Callable
...
# Import resource models for API schema generation
...
def api_schema_to_langchain_tool(api_schema, mode="generated_tool", module_name=None):
    if mode == "generated_tool":
        module = importlib.import_module("BaseAgent.tools.generated_tool." + api_schema["tool_name"] + ".api")
...
def parse_tool_calls_from_code(code: str, module2api: dict, custom_functions: dict = None) -> list[str]:
    ...
    Example:
        >>> code = "from BaseAgent.tools import analyze_data\nimport pandas as pd"
...
def format_detected_tools(detected_tool_modules: list) -> str:
    ...
    Example:
        >>> format_detected_tools([("analyze_data", "BaseAgent.tools"), ("pandas", "pandas")])
        "BaseAgent.tools → analyze_data, pandas → pandas"
    """
    tool_descriptions = []
...
def remove_emojis_from_text(text: str) -> str:
    ...
    Note:
        The function targets specific emojis used in the BaseAgent system:
        - 🔧 for tools
        - 📊 for data
        - ⚙️ for software
        - 📋 for configuration
...
def convert_markdown_to_pdf(markdown_path: str, pdf_path: str) -> None:
    ...
    try:
        ...
        <head>
            <meta charset="utf-8">
            <title>BaseAgent Conversation History</title>
